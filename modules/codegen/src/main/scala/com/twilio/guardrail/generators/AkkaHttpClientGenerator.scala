...
object AkkaHttpClientGenerator {
  ...
  object ClientTermInterp extends FunctionK[ClientTerm[ScalaLanguage, ?], Target] {
    ...
    def apply[T](term: ClientTerm[ScalaLanguage, T]): Target[T] = term match {
      case GenerateClientOperation(className, RouteMeta(pathStr, httpMethod, operation), methodName, tracing, parameters, responses) =>
        ...
        def build(methodName: String,
          ...
          val (tracingExpr, httpClientName): (Term => Term, Term.Name) =
            if (tracing)
              (op => q"""val tracingHttpClient = traceBuilder(s"$${clientName}:$${methodName}")(httpClient); ${op}""", q"tracingHttpClient")
            else
              (identity, q"httpClient")
